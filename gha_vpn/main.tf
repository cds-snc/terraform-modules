...
resource "aws_ec2_client_vpn_endpoint" "this" {
  ...
  authentication_options {
    ...
    root_certificate_chain_arn     = aws_acm_certificate.gha_vpn.arn
  }
...
resource "tls_private_key" "gha_vpn" {
  algorithm = "RSA"
}

resource "tls_self_signed_cert" "gha_vpn" {
  private_key_pem = tls_private_key.gha_vpn.private_key_pem
...
resource "aws_acm_certificate" "gha_vpn" {
  private_key      = tls_private_key.gha_vpn.private_key_pem
  certificate_body = tls_self_signed_cert.gha_vpn.cert_pem
}