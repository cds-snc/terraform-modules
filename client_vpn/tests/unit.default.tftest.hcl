...
run "default" {
  ...
  assert {
    condition     = length(aws_iam_saml_provider.client_vpn_self_service) == 0
    error_message = "aws_iam_saml_provider.client_vpn_self_service did not match expected length"
  }
