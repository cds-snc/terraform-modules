...
| [aws_cloudwatch_log_group.this_service_connect](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/cloudwatch_log_group) | resource |
...
| <a name="input_container_port_mappings"></a> [container\_port\_mappings](#input\_container\_port\_mappings) | (Optional, no default) A list of port mappings to use in the container definition | <pre>list(object({<br/>    appProtocol   = optional(string)<br/>    containerPort = number<br/>    hostPort      = optional(number)<br/>    name          = optional(string)<br/>    protocol      = optional(string)<br/>  }))</pre> | `[]` | no |
...
| <a name="input_deployment_managed_by_code_deploy"></a> [deployment\_managed\_by\_code\_deploy](#input\_deployment\_managed\_by\_code\_deploy) | (Optional, false) Determines whether ECS should let AWS CodeDeploy handle deployments | `bool` | `false` | no |
...
| <a name="input_service_connect_app_protocol"></a> [service\_connect\_app\_protocol](#input\_service\_connect\_app\_protocol) | (Optional, default `http`) The protocol used for Service Connect communication with the service. Defaults to `http`. | `string` | `"http"` | no |
