...
ECS_SERVICE_ARNS = os.getenv("ECS_SERVICE_ARNS", "").split(",")
RDS_CLUSTER_ARNS = os.getenv("RDS_CLUSTER_ARNS", "").split(",")
...
def handler(event, context):
    ...
    for ecs_arn in ECS_SERVICE_ARNS:
        ecs_service_scale(ecs_client, ecs_arn, action)

    # RDS cluster scaling
    rds_client = boto3.client('rds')
    for rds_arn in RDS_CLUSTER_ARNS:
...
def rds_cluster_scale(client, cluster_arn, action):
    ...
    response = getattr(client, function_name)(
