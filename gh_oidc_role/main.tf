...
data "aws_iam_policy_document" "oidc_assume_role_policy" {
  ...
  statement {
    ...
    principals {
      type        = "Federated"
      identifiers = [var.oidc_exists ? aws_iam_openid_connect_provider.github[0].arn : "arn:aws:iam::${data.aws_caller_identity.account_id}:oidc-provider/token.actions.githubusercontent.com"]
    }
